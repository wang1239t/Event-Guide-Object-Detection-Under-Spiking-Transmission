...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, refpoint_embed, pos_embeds, tgt, attn_mask=None):
        ...
        for lvl, (src, mask, pos_embed) in enumerate(zip(srcs, masks, pos_embeds)):
            ...
            if self.num_feature_levels > 1 and self.level_embed is not None:  # self.level_embed为一个(
                # self.num_feature_levels,c)的一个全连接接层
                lvl_pos_embed = pos_embed + self.level_embed[lvl].view(1, 1, -1)  # (b,hw,c)
            else:
                lvl_pos_embed = pos_embed
            lvl_pos_embed_flatten.append(lvl_pos_embed)
            src_flatten.append(src)
            mask_flatten.append(mask)
        src_flatten = torch.cat(src_flatten, 1)  # bs, \sum{hxw}, c  所有尺度进行拼接
        mask_flatten = torch.cat(mask_flatten, 1)  # bs, \sum{hxw}
        lvl_pos_embed_flatten = torch.cat(lvl_pos_embed_flatten, 1)  # bs, \sum{hxw}, c
        spatial_shapes = torch.as_tensor(spatial_shapes, dtype=torch.long, device=src_flatten.device) # 把list元组储存成一个（num_feature_levels，2）的tensor
        level_start_index = torch.cat((spatial_shapes.new_zeros((1,)), spatial_shapes.prod(1).cumsum(0)[:-1])) # 超长特征，每个尺度特征的起始索引
        valid_ratios = torch.stack([self.get_valid_ratio(m) for m in masks], 1) # (B,num_feature_levels,2)
...
class TransformerEncoder(nn.Module):
    ...
    def get_reference_points(spatial_shapes, valid_ratios, device):
        reference_points_list = [] # 输入为各个特征图的宽高，有效比例
        for lvl, (H_, W_) in enumerate(spatial_shapes):
            ref_y, ref_x = torch.meshgrid(torch.linspace(0.5, H_ - 0.5, H_, dtype=torch.float32, device=device),
                                          torch.linspace(0.5, W_ - 0.5, W_, dtype=torch.float32, device=device)) # 按特征图按0.5的间隔生成参考点
            ref_y = ref_y.reshape(-1)[None] / (valid_ratios[:, None, lvl, 1] * H_) # 根据掩码缩放的比例，进行拉伸
            ref_x = ref_x.reshape(-1)[None] / (valid_ratios[:, None, lvl, 0] * W_)
            ref = torch.stack((ref_x, ref_y), -1)
            reference_points_list.append(ref)
        reference_points = torch.cat(reference_points_list, 1) # 在h*w维度上拉伸
        reference_points = reference_points[:, :, None] * valid_ratios[:, None] # 超出范围的参考点拉伸回来
    ...
    def forward(self,
        ...
        if self.num_layers > 0:
            if self.deformable_encoder:
                reference_points = self.get_reference_points(spatial_shapes, valid_ratios, device=src.device) # 生成参考点
        # 暂时
        intermediate_output = []
        intermediate_ref = []
        ...
        for layer_id, layer in enumerate(self.layers):
            # main process
            dropflag = False
            if self.enc_layer_dropout_prob is not None: # dropout是否进行
                prob = random.random()
...
class DeformableTransformerEncoderLayer(nn.Module):
    ...
    def forward(self, src, pos, reference_points, spatial_shapes, level_start_index, key_padding_mask=None):
        # self attention # 传入参数，key_padding_mask为原来定义的mask_flatten
...
def build_deformable_transformer(args):
    decoder_query_perturber = None
    # 用于对解码器的查询进行噪声扰动。通过创建一个RandomBoxPerturber对象，并使用一些参数来设置噪声的规模，可以对解码器的查询进行随机扰动。
    # 这种噪声扰动的目的可能是增加模型的鲁棒性，使其能够处理具有一定噪声的输入数据。
