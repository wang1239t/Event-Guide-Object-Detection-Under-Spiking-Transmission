...
class snn_BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        self.conv1_s = tdLayer(conv3x3(inplanes, planes, stride, dilation=dilation), norm_layer(planes))
        self.conv2_s = tdLayer(conv3x3(planes, planes, dilation=dilation), norm_layer(planes))
        self.spike = LIFSpike()
...
class sew_snn_ResNet(nn.Module):
    ...
    def forward(self, x, output_layers=None):
        ...
        x = self.conv1_s(x)  # 经过一个3*3卷积核和一个bn
        x = self.spike(x)
        x = self.maxpool(x)
...
def sew_resnet50_snn(return_interm_indices=None, pretrained=None, steps=None, **kwargs):
    if return_interm_indices is None:
        return_interm_indices = [0, 1, 2, 3]
    model = sew_snn_ResNet(snn_Bottleneck, [3, 4, 6, 3], return_interm_indices, steps=steps, **kwargs)
    return model
