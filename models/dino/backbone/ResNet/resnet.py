...
def _add_output_and_check(name, x, outputs, output_layers):
...
class ResNet(nn.Module):
    def __init__(self, block, layers, output_layers, zero_init_residual: bool = False, groups: int = 1,
        ...
        # 在满足条件 zero_init_residual 为真的情况下，将模型中所有 Bottleneck 模块的第三个归一化层的权重初始化为常数 0。
        # 这种零初始化的目的是用于残差连接中，以确保初始的残差块的输出为零，从而有助于网络的训练和收敛
        if zero_init_residual:
            for m in self.modules():
                if isinstance(m, Bottleneck):
                    nn.init.constant_(m.bn3.weight, 0)  # type: ignore[arg-type]

    def _make_layer(self, block: Type[Union[Bottleneck]], planes: int, blocks: int, stride: int = 1,
    ...
    def _forward_impl(self, x: Tensor, output_layers=None):
        ...
        x = self.layer1(x)
        if _add_output_and_check('0', x, outputs, output_layers):
            return outputs, self.fg_pos

        x = self.layer2(x)
        if _add_output_and_check('1', x, outputs, output_layers):
            return outputs, self.fg_pos

        x = self.layer3(x)
        if _add_output_and_check('2', x, outputs, output_layers):
            return outputs, self.fg_pos

        x = self.layer4(x)
        if _add_output_and_check('3', x, outputs, output_layers):
            return outputs, self.fg_pos
...
def _resnet(backbone_name: str, block: Type[Union[Bottleneck]], layers: List[int], pretrained: bool, progress: bool,
            return_interm_indices: list,) -> ResNet:
